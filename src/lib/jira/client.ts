...
export async function searchJiraIssues(
  ...
  const url = `https://${site}/rest/api/3/search`;
  ...
  return withRetry(`jira search startAt=${startAt}`, async () => {
    ...
    return {
      issues: {
        ...
        nodes: (data.issues ?? []).map((issue: { key: string; fields: JiraIssueFields }) => ({
          key: issue.key,
          fields: issue.fields,
        })),
      },
...
export async function fetchAllJiraIssues(
  ...
  while (true) {
    ...
    console.log(`[jira-sync] Fetched ${response.issues.nodes.length} issues (total: ${all.length}/${response.issues.totalCount})`);

    if (all.length >= response.issues.totalCount || response.issues.nodes.length === 0) break;
    startAt += pageSize;
  }
