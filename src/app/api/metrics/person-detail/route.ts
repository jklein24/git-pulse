...
export async function GET(request: NextRequest) {
  try {
    ...
    if (userRecord?.id) {
      ...
      const weekExpr = sql<number>`((extract(epoch from ${claudeCodeUsage.date}::date)::integer + ${MONDAY_OFFSET}) - ((extract(epoch from ${claudeCodeUsage.date}::date)::integer + ${MONDAY_OFFSET}) % 604800)) - ${MONDAY_OFFSET}`;
