...
export async function GET(request: NextRequest) {
  try {
    ...
    const [prsMerged, lines, reviews, mergeTimes, aiStats, ttPerPerson] = await Promise.all([
      ...
      getReviewLoad(auth.workspace.id, startDate, endDate),
      getMergeTimePerPerson(auth.workspace.id, startDate, endDate),
      getPerPersonAiStats(auth.workspace.id, startDate, endDate).catch(() => []),
      computeAllScores(auth.workspace.id, startDate, endDate).then(aggregatePerPerson).catch(() => []),
    ]);
